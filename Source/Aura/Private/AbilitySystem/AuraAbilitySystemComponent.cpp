...
bool UAuraAbilitySystemComponent::GetDescriptionsByAbilityTag(const FGameplayTag& AbilityTag, FString& OutDescriptionPure, FString& OutActualLevelDescription, FString& OutNextLevelDescription)
{
	if (const FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		if (UAuraGameplayAbility* AuraAbility = Cast<UAuraGameplayAbility>(AbilitySpec->Ability))
		{
			OutDescriptionPure = AuraAbility->GetDescriptionPure();
			OutActualLevelDescription = AuraAbility->GetActualLevelDescription(AbilitySpec->Level);
	...
	{
		OutActualLevelDescription = FString();
	}
	else
	{
		OutActualLevelDescription = UAuraGameplayAbility::GetLockedDescription(AbilityInfo->FindAbilityInfoForTag(AbilityTag).LevelRequirement);
	}	
